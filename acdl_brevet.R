...
data_OECD <- read.csv("data/202202_EPO_App_reg_small.txt", sep = ",", stringsAsFactors = FALSE)
data_OECD <- data_OECD %>%
  mutate(year = as.numeric(substr(app_nbr, 3, 6))) %>%
  filter(ctry_code == "FR", year >= 2010, year <= 2020)

data_IPC <- read.csv("data/202202_EPO_IPC_small.txt", sep = ",", stringsAsFactors = FALSE) %>%
...
# Convertir les entreprises en majuscules
base_brevets$app_name <- toupper(base_brevets$app_name)

# Vérifier les résultats
head(base_brevets$app_name)  # Afficher les premières valeurs

# Renommer la première colonne
colnames(base_brevets)[1] <- "firm_name"
